...
{
    // функция , возвращающая двумерный вектор, в котором поменялись столбцы k и l
    // принимает двумерный вектор, число строк, число столбцов, столбец k, столбец l
    double temp;
    for (int i = 0; i < m; i++)
    {
...
{
    // функция , возвращающая двумерный вектор, в котором , был удален последний столбец
    // принимает двумерный вектор, число строк , число столбцов
    for (int i = 0; i < m; i++)
    {
...
{
    // функция печатающая слау
    // принимает двумерный вектор чисел ,вектор названия перменных, число строк, число столбцов
    for (int i = 0; i < m; i++)
    {
        for (int j = 0; j < n; j++)
        {
...
{
    // функция печатающая двумерный вектор
    // принимает - двумерный вектор, число строк ,число столбцов
    for (int i = 0; i < m; i++)
    {
        for (int j = 0; j < n; j++)
...
{
    // функция возврщает строку >= i c ненулевым элементом в данном столбце j
    // принимает двумерный вектор,число строк, число столбцов, строка i-ая, столбец j-ый
    for (int l = i; l < m; l++)
    {
        if (matrix[l][j] != 0)
        {
    ...
    return -1;
    // не найдена строка >= i с ненулевым элементом в данном столбце j

}

void swipe_string(std::vector<std::vector <double>>& matrix, int m, int n, int k, int l)
{
    // функция меняет строки l и к местами
    // принимает ссылку на матрицу, число строк , число столбцов, k -ая строка, l-ая строка
    double temp;
    for (int j = 0; j < n; j++)
    {
        temp = matrix[l][j];
    ...
    if (l != k)
    {
        std::cout << "Поменяем строки местами" << std::endl;
        print_matrix(matrix, m, n);
    }
}
void subtraction_strings(std::vector<std::vector <double>>& matrix, int m, int n, int k, int l, int j)
{
    // функция, которое вычитает из к-ой строки l -ую начиная со столбца j, так чтобы
    // в j -ом столбце в к-ой строке был элемент 0
    // принимает ссылку двумерный 
    double multiplier = matrix[k][j] / matrix[l][j];
    for (int f = j; f < n; f++)
    {
        matrix[k][f] = matrix[k][f] - matrix[l][f] * multiplier;
    }
    std::cout << "Произведем вычитание строк" << std::endl;
    print_matrix(matrix, m, n);

}
std::vector<std::vector <double>> step_matrix(std::vector<std::vector <double>> matrix, int m, int n)
{
    // возвращает матрицу, приведенной к ступенчатому виду
    // принимает матрицу , число строк ,число столбцов
    int l;
    int i = 0;
...
{
    // cчитает ранг , матрицы , считая при этом число ненулвых строк
    // в качестве аргумента должна быть ступенчатая матрица!!!!!
    // принимает матрицу , число строк, число столбцов
    int k = 0;
    // счетчик ненулевых элементов в строке
    int rang = 0;
    // считаем число не нулевых строк
...
{
    // возвращает 1 - тру - если сущесвует определитель у марицы
    // возвращает 0 - фолс - если не существует определитель у матрицы
    // принимает двумерный вектор, число строк , число столбцов
    if (m == n)
    {
        return 1;
...
{
    // cчитает определитель матрицы , считая произведение элементов на диагонали 
    // в качестве аргумента должна быть ступенчатая матрица!!!!!
    // принимает матрицу , число строк, число столбцов
    double p = 1;
    for (int i = 0; i < n; i++)
